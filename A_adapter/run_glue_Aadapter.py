...
def main():
    ...
    # Add adapter and activate adapter structure
    model.add_adapter(args.task_name, config="houlsby")
    model.add_classification_head(
        args.task_name,
        num_labels=num_labels
    )
    # Freeze parameters and activate adapter
    ...
    test_loss = []
    # init Aadapter
    adv = Aadapter(adv_K=3, adv_lr=1e-1, adv_init_mag=2e-2, adv_max_norm=1.0, adv_norm_type='l2', base_model='bert')
    print(f"Training by Adversarial adapter. Datasets: {args.task_name.upper()}")
    
    for epoch in range(args.num_train_epochs):
        model.train()
        for step, batch in enumerate(train_dataloader):
            # get init word embeddings
            embeds_init = adv.get_embeds_init(model, batch)
            # initialize delta
            delta = adv.getDelta(embeds_init, batch)
            ...
            for astep in range(adv.adv_K):
                ...
                # update
                # get new delta
                delta = adv.updateDelta(delta, delta_grad, embeds_init)
                # get new embedding
                batch['input_ids'] = batch_store
                embeds_init = adv.get_embeds_init(model, batch)
            
            KL_loss = adv.KL_term(model, batch_store, batch)
            KL_loss.backward()
