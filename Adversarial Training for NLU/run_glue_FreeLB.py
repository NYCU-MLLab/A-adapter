...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="Close dropout. More information in FreeLB chapter 3.3"
    )
    parser.add_argument(
        "--use_slow_tokenizer",
        action="store_true",
...
def main():
    ...
    config = AutoConfig.from_pretrained(
        args.model_name_or_path, num_labels=num_labels, finetuning_task=args.task_name, attention_probs_dropout_prob=args.attention_probs_dropout_prob)
    ...
    # set freeLb
    freelb = FreeLB(adv_K=3, adv_lr=1e-1, adv_init_mag=2e-2,
                    adv_max_norm=1.0, adv_norm_type='l2', base_model='bert')
    print(
        f"Adversarial training by FreeLB. Datasets: {args.task_name.upper()}")
    for epoch in range(args.num_train_epochs):
        model.train()
        for step, batch in enumerate(train_dataloader):
            # get init word embeddings
            embeds_init = freelb.get_embeds_init(model, batch)
            # initialize delta
            delta = freelb.getDelta(embeds_init, batch)

            for astep in range(freelb.adv_K):
                # forward and backward
                delta.requires_grad_()
                # L(X + delta)
                batch['inputs_embeds'] = embeds_init + delta
                batch_store = batch['input_ids']
                ...
                # update
                # get new delta
                delta = freelb.updateDelta(delta, delta_grad, embeds_init)
                # get new embedding
                batch['input_ids'] = batch_store
                embeds_init = freelb.get_embeds_init(model, batch)
    ...
    f = open("loss_record.txt", "a", encoding="utf-8")
