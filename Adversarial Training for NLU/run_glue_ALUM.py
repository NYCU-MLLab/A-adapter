...
def main():
    ...
    alum = ALUM(adv_lr=1e-3, alpha=1, noise_rate=1e-5, adv_norm_type='l2', base_model='bert')
    print(f"Adversarial training by VAT(ALUM ver.) Datasets: {args.task_name.upper()}")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # hidden_status = model.bert.embeddings.word_embeddings(batch["input_ids"])
            ...
            adv_loss = alum.VAT(model, batch, logits)
            if adv_loss:
                loss = loss + adv_loss
            # Line.9
