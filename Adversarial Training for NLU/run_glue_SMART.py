...
def main():
    ...
    print(f"Start training by SMART-smoothness-inducing. Using dataset: {args.task_name.upper()}")
    smart = SMARTLoss(model)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            embed = model.bert.embeddings(input_ids)
            state = smart.eval(embed)
            Rs = smart(embed, state)
            # total loss = L(θ) + λs*Rs(θ)
            loss = loss + lambdas * Rs
