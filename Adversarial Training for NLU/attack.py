...
class SMARTLoss(nn.Module):
    ...
    def loss(self, inputs, target, reduction='batchmean', loss_type="Jeffray"):
        """
        Default: Jeffray Div, or using JS Div.
        """
        inputs = inputs.float()
        target = target.float()
        if loss_type != "JS":
    ...
    def forward(self, embed: Tensor, state: Union[Tensor, List[Tensor]]) -> Tensor:
        ...
        for i in range(self.inner_loop + 2):
            ...
            states_perturbed = self.to_list(self.eval_fn(adversarial_example))
            adv_loss = 0
            ...
            delta_gradient = torch.autograd.grad(adv_loss, delta)[0]
            # Gradient ascent 
