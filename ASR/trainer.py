...
class VoiceAttack(object):
    '''
    TRADES style for voice
    '''

    def __init__(self, adv_K=2, adv_lr=1e-1, adv_init_mag=2e-2, adv_max_norm=1.0, adv_norm_type='linf', base_model='Hubert'):
        """
        Voice Adversarial training by adv_augment, we temporary found no KL_regular get better result.
        """
        self.adv_K = adv_K
        self.adv_lr = adv_lr
    ...
    def getDelta(self, embeds_init, batch):
        ...
        length = embeds_init.shape[1]
        # check shape
        attention_mask = attention_mask.view(-1, length)
        embeds_init = embeds_init.view(-1, length)
        # create masks
        if self.adv_init_mag > 0:
            ...
            delta = delta * input_mask
        else:
            # no delta
            delta = torch.zeros_like(embeds_init)
        
        return delta.view(batch_size, length)

    def updateDelta(self, delta, delta_grad, embeds_init):
        batch_size = delta.shape[0]
        length = delta.shape[1]
        delta = delta.view(-1, length)
        delta_grad = delta_grad.view(-1, length)
        denorm = torch.norm(delta_grad, dim=1, p=float("inf")).view(-1, 1)
...
class Trainer:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss = self.compute_adv_loss(model, inputs)
    ...
    def compute_adv_loss(self, model, inputs, return_outputs=False):
        """
        For adversarial training, we set inner loop 2 times as default.
        """
        if self.label_smoother is not None and "labels" in inputs:
            labels = inputs.pop("labels")
        else:
            labels = None
        # init adversarial training config, include delta and init input values  
        adv = VoiceAttack(adv_K=3, adv_lr=1e-1, adv_init_mag=2e-2, adv_max_norm=1.0, adv_norm_type='linf')
        init_input_values = input_values = inputs["input_values"]
        delta = adv.getDelta(init_input_values, inputs)
        # inner loop
        for astep in range(adv.adv_K):
            # forward and backward
            delta.requires_grad_()
            # L(X + delta)
            inputs['input_values'] = input_values + delta
            ...
            if astep == adv.adv_K - 1:
                # further updates on delta
                break

            # get gradient on delta
            delta_grad = delta.grad.clone().detach()
            # update
            # get new delta
            delta = adv.updateDelta(delta, delta_grad, input_values)
            # get new embedding
